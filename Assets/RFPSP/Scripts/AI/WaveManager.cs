...
public class MultiDimensionalInt
{
	...
	[Tooltip("Limit of NPCs from the spawner that can be active in the scene at once.")]
	public int[] NpcLoads;
...
public class WaveManager : MonoBehaviour {
	...
	void Start () {
		...
		playerAudio = Camera.main.GetComponent<CameraKick>().playerObj.GetComponent<FPSPlayer>().otherfx;
	...
	public IEnumerator StartWave(){
		...
		if(waveNumber > 1 || lastWave){
			...
			if(waveEndFx && !fxPlayed2){
				fxPlayed2 = true;
				playerAudio.PlayOneShot(waveEndFx, 1.0f);
			}
		...
		while(true){
			...
			if(countDown <= 0.0f){
				...
				if(waveStartFx && !fxPlayed){
					fxPlayed = true;
					fxPlayed2 = false;
					playerAudio.PlayOneShot(waveStartFx, 1.0f);
				}
