    ...
    IEnumerator Detonate() {
		...
		if(explosionEffect){
		    foreach (Transform child in explosionEffect){//emit all particles in the particle effect game object group stored in explosionEffect var
				...
				child.GetComponent<ParticleEmitter>().transform.rotation = Quaternion.FromToRotation(Vector3.up, Vector3.up);//rotate effects so they are vertical
					...
					switch(hitColliders[i].GetComponent<Collider>().gameObject.layer){
						case 13://hit object is an NPC
							if(hitColliders[i].GetComponent<Collider>().gameObject.GetComponent<CharacterDamage>()){
								hitColliders[i].GetComponent<Collider>().gameObject.GetComponent<CharacterDamage>().ApplyDamage(explosionDamageAmt, Vector3.zero, myTransform.position, null, false);
							}
							break;
