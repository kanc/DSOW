...
public class WaterZone : MonoBehaviour {
	...
	void Start () {
		...
		if(waterPlane){waterPlane.gameObject.SetActive(false);}
		...
		&& colTransform.position.y > myTransform.GetComponent<Collider>().bounds.max.y - 0.3f)){
			...
			if(colTag == "Player"){
				...
				if(FootstepsComponent.waterLand){
					PlayAudioAtPos.instance.PlayClipAt(FootstepsComponent.waterLand, col.gameObject.transform.position, 1.0f, 0.0f, 1.0f);//play splash sound
				}
			}else{
				if(FootstepsComponent.waterLand){
					PlayAudioAtPos.instance.PlayClipAt(FootstepsComponent.waterLand, colTransform.position, 1.0f, 1.0f, 1.0f);//play splash sound
				}
			}
					...
					if(splashTrailTime + 0.075f < Time.time){
						if(splashTrail){
						    foreach (Transform child in splashTrail.transform){//emit all particles in the particle effect game object group stored in impactObj var
								...
								child.GetComponent<ParticleEmitter>().transform.rotation = Quaternion.FromToRotation(Vector3.up, splashTrail.transform.up);
								child.GetComponent<ParticleEmitter>().Emit();//emit the particle(s)
							}
						}
