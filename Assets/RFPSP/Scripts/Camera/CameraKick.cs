...
//	private float waveslice;
	...
	private float camSmoothSpeed = 0.075f;
	...
	//		}
			...
			//lerp camera normally if not on elevator or moving platform
			...
			||(!FPSPlayerComponent.removePrefabRoot && playerObj.transform.parent == FPSWalkerComponent.mainObj.transform)){
				...
				lerpSpeedAmt = Mathf.MoveTowards(lerpSpeedAmt, 0.025f, Time.deltaTime);
			}
			...
			tempLerpPos = Vector3.SmoothDamp(tempLerpPos, targetPos, ref dampVel, lerpSpeedAmt, Mathf.Infinity, Time.deltaTime);
			...
			if(FPSWalkerComponent.sprintActive){
				strafeRollAmt = IronsightsComponent.sprintStrafeRoll;	
			}else{
				strafeRollAmt = IronsightsComponent.walkStrafeRoll;	
			}
			...
			Vector3 tempCamAngles = new Vector3(mainCameraTransform.localEulerAngles.x - bobAngles.x + (CameraPitchAmt * Time.deltaTime * 75.0f) ,
												...
												mainCameraTransform.localEulerAngles.z - bobAngles.z + (CameraRollAmt * Time.deltaTime * 75.0f) 
																									 - (FPSWalkerComponent.leanAmt * 3.0f * Time.deltaTime * returnSpeed) 
																									 - (FPSWalkerComponent.inputX * strafeRollAmt * Time.deltaTime * returnSpeed)); 
