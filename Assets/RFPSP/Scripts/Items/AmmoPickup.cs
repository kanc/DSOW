...
public class AmmoPickup : MonoBehaviour {
	...
	public void PickUpItem(){
		...
		if (weaponObj.GetComponent<WeaponBehavior>().ammo < weaponObj.GetComponent<WeaponBehavior>().maxAmmo) {
			...
			//play pickup sound
			if(pickupSound){
				PlayAudioAtPos.instance.PlayClipAt(pickupSound, myTransform.position, 0.75f, 1.0f, 1.0f);
			}
			...
			if(fullSound){
				PlayAudioAtPos.instance.PlayClipAt(fullSound, myTransform.position, 0.75f, 1.0f, 1.0f);
			}	
		}
	}
}