...
public class WeaponEffects : MonoBehaviour {
	...
	public float tracerDist = -0.2f;
	public float tracerDistSwim = -0.2f;
	...
	public void Start(){
		...
		FPSWalkerComponent = Camera.main.transform.GetComponent<CameraKick>().playerObj.GetComponent<FPSRigidBodyWalker>();
	}
	...
	public void ImpactEffects ( RaycastHit hit, bool NpcAttack ){
		WeaponBehaviorComponent = PlayerWeaponsComponent.CurrentWeaponBehaviorComponent;
		if(WeaponBehaviorComponent.meleeSwingDelay == 0 || NpcAttack){//not a melee weapon
	    ...
	    foreach (Transform child in impactObj.transform){//emit all particles in the particle effect game object group stored in impactObj var
			Transform childParticleTransform = child.GetComponent<ParticleEmitter>().transform;//cache child particle transform ref for efficiency
		...
		PlayAudioAtPos.instance.PlayClipAt(hitSound, hit.point, hitVolumeAmt, 1.0f, 1.0f);
	...
	public void BulletTracers ( Vector3 direction, Transform MuzzleFlashTransform){
		//Draw Bullet Tracers
		if(!FPSWalkerComponent.holdingBreath){
			if (tracerParticles) {
				...
				tracerParticles.transform.position = MuzzleFlashTransform.position + MuzzleFlashTransform.forward * tracerDist;
			...
			if (bubbleParticles) {
				bubbleParticles.transform.position = MuzzleFlashTransform.position - MuzzleFlashTransform.forward * tracerDistSwim;
