...
public class Ironsights : MonoBehaviour {
	...
	private VerticalBob VerticalBob;
	...
	[Tooltip("True if the camera should bob vertically to simulate player breathing.")]
	public bool cameraIdleBob = true;//true if camera should bob slightly up and down when idle to simulate player breathing 
	[Tooltip("True if the camera should bob vertically to simulate floating in water.")]
	...
	void Update (){
		...
		if(Time.timeScale > 0 && Time.deltaTime > 0){//allow pausing by setting timescale to 0
			...
			if(FPSWalker.moving){
				...
				&& !FPSPlayerComponent.zoomed
					...
					&& !FPSWalker.jumping){//actually sprinting now
						...
						if(FPSWalker.inputY != 0 && !FPSWalker.prone){
							nextFov = sprintFov;
						}else{
							nextFov = defaultFov;	
						}
				...
				if(FPSPlayerComponent.zoomed && WeaponBehaviorComponent.meleeSwingDelay == 0) {
					idleX = Mathf.Sin(Time.time * 1.25f) * 0.0005f;
					idleY = Mathf.Sin(Time.time * 1.5f) * 0.0005f;
				}else{
					if(!FPSWalker.swimming){
						idleX = Mathf.Sin(Time.time * 1.25f) * 0.0012f;
						idleY = Mathf.Sin(Time.time * 1.5f) * 0.0012f;
					}else{
						idleX = Mathf.Sin(Time.time * 1.25f) * 0.003f;
						idleY = Mathf.Sin(Time.time * 1.5f) * 0.003f;	
					}
				}
			}
			...
			&& WeaponBehaviorComponent.reloadLastStartTime + WeaponBehaviorComponent.reloadLastTime < Time.time){
				...
				zDampSpeed = 0.1f;
				//X pos with idle movement
				nextPosX = side + idleX + sideMove + horizontalGunPosAmt + (FPSWalker.leanAmt / 60.0f)
					//also add X bobbing amounts here so they get smoothed to remove any small glitches in position
					...
					if (FPSWalker.grounded) {
						if (!FPSWalker.crouched && !FPSWalker.prone && FPSWalker.midPos >= FPSWalker.standingCamHeight){
							...
							if(FPSWalker.crouched){
								...
								////prone bob amounts////
								//horizontal bobbing 
								if(GBamt < ProneHorizontalBobAmount){GBamt += 0.005f * deltaAmount;}
								if(GBamt > ProneHorizontalBobAmount){GBamt -= 0.005f * deltaAmount;;}
								//vertical bobbing
								if(HBamt < ProneVerticalBobAmount){HBamt += 0.005f * deltaAmount;}
								if(HBamt > ProneVerticalBobAmount){HBamt -= 0.005f * deltaAmount;}
								//pitching
								if(HPamt < ProneBobPitchAmount){HPamt += 0.0075f * deltaAmount;}
								if(HPamt > ProneBobPitchAmount){HPamt -= 0.0075f * deltaAmount;}
								//rolling
								if(HRamt < ProneBobRollAmount){HRamt += 0.0075f * deltaAmount;}
								if(HRamt > ProneBobRollAmount){HRamt -= 0.0075f * deltaAmount;}
								//yawing
								if(HYamt < ProneBobYawAmount){HYamt += 0.0075f * deltaAmount;}
								if(HYamt > ProneBobYawAmount){HYamt -= 0.0075f * deltaAmount;}
