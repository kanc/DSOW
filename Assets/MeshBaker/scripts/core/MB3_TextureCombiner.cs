...
//			new ShaderTextureProperty("_DetailNormalMap",true),
		...
		public class MeshBakerMaterialTexture{
			...
			public MeshBakerMaterialTexture(Texture2D tx, Vector2 o, Vector2 s, Vector2 oUV, Vector2 sUV, Color c){
				t = tx;
				offset = o;
				scale = s;
				obUVoffset = oUV;
	    ...
	    *  <remarks>Combines meshes and generates texture atlases</remarks> */		
		...
		bool __CombineTexturesIntoAtlases(ProgressUpdateDelegate progressInfo, MB_AtlasesAndRects resultAtlasesAndRects, Material resultMaterial, List<ShaderTextureProperty> texPropertyNames, List<GameObject> objsToMesh, List<Material> allowedMaterialsFilter, MB2_EditorMethodsInterface textureEditorMethods){
			...
			*/
			...
			bool[] allTexturesAreNull = new bool[texPropertyNames.Count];
			int _padding = __Step2_CalculateIdealSizesForTexturesInAtlasAndPadding(distinctMaterialTextures,texPropertyNames,allTexturesAreNull);	
						
			__Step3_BuildAndSaveAtlasesAndStoreResults(progressInfo,distinctMaterialTextures,texPropertyNames,allTexturesAreNull,_padding,textureEditorMethods,resultAtlasesAndRects,resultMaterial);
					...
					for (int j = 0; j < texPropertyNames.Count; j++){
						...
						if (mat.HasProperty(texPropertyNames[j].name)){
							...
							if (txx != null){
								...
								colorIfNoTexture = GetColorIfNoTexture(mat,texPropertyNames[j]);
							}
						...
						mts[j] = new MeshBakerMaterialTexture(tx,offset,scale,obUVoffset,obUVscale,colorIfNoTexture);
					}
		...
		int __Step2_CalculateIdealSizesForTexturesInAtlasAndPadding(List<MB_TexSet> distinctMaterialTextures,
			...
			if (distinctMaterialTextures.Count == 1){
				...
				if (allTexturesAreNull.Length != texPropertyNames.Count){
					Debug.LogError("allTexturesAreNull array must be the same length of texPropertyNames.");
				}
				for(int i = 0; i < distinctMaterialTextures.Count; i++){
			...
			// check if all textures are null for each atlas
			for (int i = 0; i < texPropertyNames.Count; i++){
				bool foundAtLeastOneTex = false;
				for (int j = 0; j < distinctMaterialTextures.Count; j++){
					if (distinctMaterialTextures[j].ts[i].t != null) foundAtLeastOneTex = true;
				}
				allTexturesAreNull[i] = !foundAtLeastOneTex;
			}
			return _padding;
		}
		
		void __Step3_BuildAndSaveAtlasesAndStoreResults(ProgressUpdateDelegate progressInfo, List<MB_TexSet> distinctMaterialTextures, List<ShaderTextureProperty> texPropertyNames, bool[] allTexturesAreNull, int _padding, MB2_EditorMethodsInterface textureEditorMethods, MB_AtlasesAndRects resultAtlasesAndRects, Material resultMaterial){
				...
				for (int i = 0; i < distinctMaterialTextures.Count; i++){
					...
					for (int j = 0; j < txs.ts.Length; j++){
						if (txs.ts[j].t != null){
							...
							if (allTexturesAreNull[j]){
								report.Append ("no atlas will be created all textures null]\n");
							} else {
								report.AppendFormat("a 2x2 texture will be created with color {0}]\n",txs.ts[j].colorIfNoTexture);
							}
						}
					}
					report.AppendLine("");
			...
			if (_packingAlgorithm == MB2_PackingAlgorithmEnum.UnitysPackTextures){
				uvRects = __CreateAtlasesUnityTexturePacker(progressInfo, numAtlases, distinctMaterialTextures, texPropertyNames, allTexturesAreNull, resultMaterial, atlases, textureEditorMethods, _padding);
			} else {
				uvRects = __CreateAtlasesMBTexturePacker(progressInfo, numAtlases, distinctMaterialTextures, texPropertyNames, allTexturesAreNull, resultMaterial, atlases, textureEditorMethods, _padding);
			}
		...
		Rect[] __CreateAtlasesMBTexturePacker(ProgressUpdateDelegate progressInfo, int numAtlases, List<MB_TexSet> distinctMaterialTextures, List<ShaderTextureProperty> texPropertyNames, bool[] allTexturesAreNull, Material resultMaterial, Texture2D[] atlases, MB2_EditorMethodsInterface textureEditorMethods, int _padding){
			Rect[] uvRects;
			if (distinctMaterialTextures.Count == 1){
				...
				for (int i = 0; i < numAtlases; i++){
					...
					if (allTexturesAreNull[i]){
						atlas = null;
						if (LOG_LEVEL >= MB2_LogLevel.debug) Debug.Log("No creating atlas for " + texPropertyNames[i].name + " because all textures are null.");
					} else {
						GC.Collect();
...
//						}
		...
		Rect[] __CreateAtlasesUnityTexturePacker(ProgressUpdateDelegate progressInfo, int numAtlases, List<MB_TexSet> distinctMaterialTextures, List<ShaderTextureProperty> texPropertyNames, bool[] allTexturesAreNull, Material resultMaterial, Texture2D[] atlases, MB2_EditorMethodsInterface textureEditorMethods, int _padding){
			Rect[] uvRects;
			if (distinctMaterialTextures.Count == 1){
				if (LOG_LEVEL >= MB2_LogLevel.debug) Debug.Log("Only one image per atlas. Will re-use original texture");
				uvRects = new Rect[1];
...
//						Vector2 obUVoffset = Vector2.zero; 
		...
		public Color GetColorIfNoTexture(Material mat, ShaderTextureProperty texProperty){
			if (texProperty.isNormalMap){
				return new Color(.5f,.5f,1f); //neutral normalmap color
			...
			} else if (texProperty.name.Equals ("_SpecMap") ||
			           texProperty.name.Equals ("_SpecGlossMap")){
				...
				if (mat != null && mat.HasProperty("_Metallic")){
					try{ //need try because can't garantee _Color is a color
						...
						float mv = mat.GetFloat("_Metallic");
						Color metalColor = new Color(mv,mv,mv,glossiness);
						return metalColor;
					} catch(Exception){}
				}
				...
				return Color.clear; //alpha must be zero.
			}
